...
def solve_multiple_planes_ocp(k_values: np.array, init_estimate=None, h_final=850):
    ...
    for k_value in k_values:
        ...
        Wx_expressions += [Wx_expr]
        Wh_expr = wind_h(x_sym, h_sym, k_value)
        Wh_expressions += [Wh_expr]
        dWx_dx_functions += [
            ca.Function("dWx_dx", [x_sym], [ca.gradient(Wx_expr, x_sym)])
        ]
        dWh_dx_functions += [
            ca.Function("dWh_dx", [x_sym, h_sym], [ca.gradient(Wh_expr, x_sym)])
        ]
        dWh_dh_functions += [
            ca.Function("dWh_dh", [x_sym, h_sym], [ca.gradient(Wh_expr, h_sym)])
        ]
...
def aircraft_ode(X, u_, t_, dWx_dx_fun, dWh_dx_fun, dWh_dh_fun, k_value):
...
def rk4_step(f, xk, uk, tk, dt, dWx_dx_fun, dWh_dx_fun, dWh_dh_fun, k_value):
    k1 = f(xk, uk, tk, dWx_dx_fun, dWh_dx_fun, dWh_dh_fun, k_value)
    k2 = f(
        xk + dt / 2 * k1, uk, tk + dt / 2, dWx_dx_fun, dWh_dx_fun, dWh_dh_fun, k_value
    )
    k3 = f(
        xk + dt / 2 * k2, uk, tk + dt / 2, dWx_dx_fun, dWh_dx_fun, dWh_dh_fun, k_value
    )
    k4 = f(xk + dt * k3, uk, tk + dt, dWx_dx_fun, dWh_dx_fun, dWh_dh_fun, k_value)
