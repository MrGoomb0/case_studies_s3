...
def simulate_trajectories(u_opt: list[float], M: int, plot=True):
    '''
    Input:
        u_opt (list with floats): The computed optimal control
        M (int): Number of MC samples
    Return:
        dict: Lists of k-values violating the x2dotdot and alphamax constraints
    ...
    for k_val in k_samples:
        ...
        for i in range(params.nu):
            ...
            if x2dotdot > 10 * params.g:
                violated_high = True
            if abs(x5) > params.alphamax:
                violated_alpha = True

        if violated_low:
            violating_ks['x2dotdot_low'].append(k_val)
        if violated_high:
            violating_ks['x2dotdot_high'].append(k_val)
        if violated_alpha:
            violating_ks['alpha_violation'].append(k_val)
