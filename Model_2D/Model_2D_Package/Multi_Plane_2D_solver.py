...
def solve_ocp_multi_plane_min_h(k_values):
    ...
    B2 = -51 * exp(-params.c * (x1 - 2300) ** 4) # why 30 here
...
def u_opt_return(w_opt):
    u_opt = w_opt[:params.nu]
    return u_opt


def plot_multi_plane_2D(w_opt,k_values):
    N = params.nu
    tf = params.tf
    nk = len(k_values)
...
def solve_ocp_multi_plane_min_h_initial_geuss_help(k_values,state_geuss):
    nk = len(k_values)
    ...
    for i in range(nk):
        ...
        for k in range(params.nu):
            ...
            start_idx = 5 * (k + 1)
            end_idx = start_idx + 5
            w0 += state_geuss[start_idx:end_idx]
            lbw += [-inf, 0, -inf, -inf, -inf]
            ubw += [inf, inf, inf, inf, 17.2 * pi / 180]
            g += [Xk_end - Xk]
            lbg += [0, 0, 0, 0, 0]
...
def solve_ocp_multi_plane_min_h_k_sym(k_values):
    ...
    B1 = params.d * x1 ** 3 + params.e * x1 ** 4
    B2 = -51 * exp(fmin(-params.c * (x1 - 2300) ** 4, 30)) # why 30 here
    B3 = params.d * (4600 - x1) ** 3 + params.e * (4600 - x1) ** 4
    B4 = 0
    B = if_else(x1 <= 500, B1,
                if_else(x1 <= 4100, B2,
...
def reconstruction_multi_plane_2D(u_opt, k_value, multipler):
    ...
    return {'x': X[0::5],'h': X[1::5],'V': X[2::5],'gamma': X[3::5],'alpha': X[4::5],'u':U,'t_grid':T}



def simulate_trajectories(u_opt, params, N, dxt, M=1000, plot=True):
    def wind_fields(x1, x2, k):
        ...
        if x1 <= 500:
            ...
            B = -51 * np.exp(min(-params.c * (x1 - 2300) ** 4, 30))
        elif x1 <= 4600:
    ...
    def dynamics(x, u, t, k):
        ...
        x3_safe = max(x3, 1e-3)
        x3dot = (T * np.cos(x5 + params.delta) - D) / params.m - params.g * np.sin(x4) - (wxdot * np.cos(x4) + whdot * np.sin(x4))
...
def simulate_single_trajectory(u_opt, params, N, dxt, k_val):
    def wind_fields(x1, x2, k):
        # (same as before)
        if x1 <= 500:
            A = -50 + params.a * x1 ** 3 + params.b * x1 ** 4
